...
func (mfa *MFAManager) VerifyBackupCode(backupCodes []string, code string) (bool, []string, error) {
	...
	for i, storedCode := range backupCodes {
		...
		if code == normalizedStored {
			// Remove used backup code (copy to avoid mutating the original slice)
			remainingCodes := make([]string, 0, len(backupCodes)-1)
			remainingCodes = append(remainingCodes, backupCodes[:i]...)
			remainingCodes = append(remainingCodes, backupCodes[i+1:]...)
