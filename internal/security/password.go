...
func (pm *PasswordManager) GenerateSecureRandomPassword(length int) (string, error) {
	...
	if length >= 4 {
		password[0] = uppercase[randomBytes[0]%byte(len(uppercase))]
		password[1] = lowercase[randomBytes[1]%byte(len(lowercase))]
		password[2] = digits[randomBytes[2]%byte(len(digits))]
