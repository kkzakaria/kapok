...
func (al *AuditLogger) InitializeAuditTable(ctx context.Context) error {
	query := fmt.Sprintf(`
		...
		CREATE INDEX IF NOT EXISTS idx_audit_timestamp ON %s (timestamp DESC);
		CREATE INDEX IF NOT EXISTS idx_audit_tenant ON %s (tenant_id, timestamp DESC);
		CREATE INDEX IF NOT EXISTS idx_audit_user ON %s (user_id, timestamp DESC);
		CREATE INDEX IF NOT EXISTS idx_audit_event_type ON %s (event_type, timestamp DESC);
	...
	`, al.tableName, al.tableName, al.tableName, al.tableName, al.tableName, al.tableName, al.tableName)
